"""Evaluate TUTR prediction bundles with the standard ETH-UCY AFC bank."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate TUTR exported bundle with AFC metrics.")
...
def main() -> None:
    ...
    for index, record in enumerate(records):
        ...
        if (index + 1) % 100 == 0:
            print(f"[evaluate_tutr_afc] evaluated_records={index + 1}/{len(records)}")
    ...
    output = {
        "meta": {
            "script": "trustmoe_traj.scripts.evaluate_tutr_afc",
            "baseline": "TUTR",
