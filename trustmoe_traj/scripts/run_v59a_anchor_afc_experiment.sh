...
# Train and evaluate V59A Anchor-preserving AFC-aware residual refinement.
# The script intentionally resolves slow/data paths from existing V58M eval JSONs,
# while the new V59A refiner and quality scorer checkpoints are passed explicitly.
...
RUN_ID="${RUN_ID:-$(date +%Y%m%d)_v59a_anchor_afc}"
...
run_dataset_seed() {
  ...
  local refiner_ckpt="$refiner_dir/v59a_refiner_best.pt"
  ...
  if [[ "$TRAIN_QUALITY" == "1" && ( "$FORCE" == "1" || ! -f "$quality_ckpt" ) ]]; then
    ...
    log "TRAIN V59A quality scorer dataset=$dataset seed=$seed"
  ...
  if [[ "$RUN_EVAL" == "1" ]]; then
    ...
    for threshold in $THRESHOLDS; do
      ...
      eval_prefix="v59a_${dataset}_anchor_afc_${tag}"
      for split in $SPLITS; do
        ...
        log "EVAL V59A dataset=$dataset seed=$seed split=$split threshold=$threshold"
...
log "START V59A Anchor-AFC experiment"
log "MAIN=$MAIN"
log "PY=$PY"
...
if [[ "$RUN_SUMMARY" == "1" ]]; then
  ...
  "$PY" -m trustmoe_traj.scripts.analyze_v58m_afc_mvp \
    ...
    --eval-prefix-template "v59a_{dataset}_anchor_afc_{tag}" 2>&1 | tee "$LOG_ROOT/analysis_afc_mvp.log"
  "$PY" -m trustmoe_traj.scripts.analyze_v58m_qd_supplement \
    ...
    --threshold-tags "$(threshold_tags_csv)" \
    --run-prefix-template "${RUN_ID}_{dataset}" \
    --eval-prefix-template "v59a_{dataset}_anchor_afc_{tag}" 2>&1 | tee "$LOG_ROOT/analysis_qd_supplement.log"
fi
