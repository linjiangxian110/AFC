"""Aggregate V18-A student-integrated train/eval JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize V18-A student-integrated experiment outputs.")
    parser.add_argument("--project-root", type=str, default=str(PROJECT_ROOT))
    parser.add_argument("--run-prefix", type=str, required=True)
    parser.add_argument("--run-name", type=str, default="adapter")
...
def _seed_row(args: argparse.Namespace, project_root: Path, seed: int, splits: Sequence[str]) -> Dict[str, Any]:
    ...
    if isinstance(best_val, Mapping):
        row["cache_val_student_integrated_FDE_min"] = best_val.get("student_integrated_FDE_min")
        row["cache_val_dFDE_min"] = best_val.get("dFDE_min")
        row["cache_val_gate_mean"] = best_val.get("gate_mean")
    ...
    for split in splits:
        ...
        if isinstance(metrics, Mapping):
            for metric in METRICS:
                adapted = _metric(metrics, "student_integrated_pred", metric)
                fast = _metric(metrics, "fast_pred", metric)
                split_row[f"student_integrated_{metric}"] = adapted
                split_row[f"fast_{metric}"] = fast
                split_row[f"d{metric}"] = None if adapted is None or fast is None else adapted - fast
            split_row["slow_FDE_min"] = _metric(metrics, "slow_pred", "FDE_min")
            split_row["head_latency_avg_ms"] = _num(metrics.get("student_integrated_head_latency_avg_ms"))
            split_row["gate_mean"] = _num(metrics.get("student_integrated_gate_mean"))
            split_row["delta_l2_mean"] = _num(metrics.get("student_integrated_delta_l2_mean"))
...
def _aggregate(rows: Sequence[Mapping[str, Any]], splits: Sequence[str]) -> Dict[str, Any]:
    ...
    for split in splits:
        ...
        split_row["mean_head_latency_avg_ms"] = _mean(row.get(key, {}).get("head_latency_avg_ms") for row in rows)
...
def _render(rows: Sequence[Mapping[str, Any]], aggregate: Mapping[str, Any], splits: Sequence[str]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f"cache-val student_integrated_FDE_min: {row.get('cache_val_student_integrated_FDE_min')}")
        lines.append(f"cache-val dFDE_min: {_fmt(row.get('cache_val_dFDE_min'), signed=True)}")
        lines.append(f"cache-val gate: {row.get('cache_val_gate_mean')}")
        lines.append(f"cache-val delta_l2: {row.get('cache_val_delta_l2_mean')}")
        lines.append("")
        for split in splits:
            official = row.get(f"official_{split}", {})
            lines.append(f"-- official {split} StudentIntegrated - Fast --")
            for metric in METRICS:
                lines.append(
                    f"d{metric}: {_fmt(official.get(f'd{metric}'), signed=True)}  "
                    f"student_integrated={_fmt(official.get(f'student_integrated_{metric}'))}  "
    ...
    for split in splits:
        ...
        lines.append(f"mean head_latency_avg_ms: {_fmt(mean.get('mean_head_latency_avg_ms'))}")
    return "\n".join(lines).rstrip() + "\n"
...
def main() -> None:
    ...
    payload = {
        "meta": {
            "script": "trustmoe_traj.scripts.summarize_student_integrated_experiment",
