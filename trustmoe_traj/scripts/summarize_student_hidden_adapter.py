"""Aggregate student hidden-token adapter experiment outputs."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize student hidden-token adapter outputs.")
    parser.add_argument("--project-root", type=str, default=str(PROJECT_ROOT))
    parser.add_argument("--run-prefix", type=str, required=True)
    parser.add_argument("--run-name", type=str, default="student_hidden_adapter")
    parser.add_argument("--eval-file-prefix", type=str, default="v19a_official")
...
def _seed_row(args: argparse.Namespace, project_root: Path, seed: int, splits: Sequence[str]) -> Dict[str, Any]:
    ...
    if isinstance(best_val, Mapping):
        row["cache_val_hidden_FDE_min"] = best_val.get("finetuned_FDE_min")
        ...
        row["cache_val_hidden_gate_mean"] = best_val.get("hidden_gate_mean")
        row["cache_val_hidden_delta_l2_mean"] = best_val.get("hidden_delta_l2_mean")
    ...
    for split in splits:
        ...
        if isinstance(metrics, Mapping):
            for metric in METRICS:
                hidden = _metric(metrics, "hidden_adapter_pred", metric)
                fast = _metric(metrics, "fast_pred", metric)
                split_row[f"hidden_adapter_{metric}"] = hidden
                split_row[f"fast_{metric}"] = fast
                split_row[f"d{metric}"] = None if hidden is None or fast is None else hidden - fast
            split_row["slow_FDE_min"] = _metric(metrics, "slow_pred", "FDE_min")
            split_row["hidden_latency_avg_ms"] = _num(metrics.get("hidden_adapter_pred_latency_avg_ms"))
            split_row["fast_latency_avg_ms"] = _num(metrics.get("fast_pred_latency_avg_ms"))
            split_row["gate_mean"] = _num(metrics.get("hidden_adapter_gate_mean"))
            split_row["delta_l2_mean"] = _num(metrics.get("hidden_adapter_delta_l2_mean"))
...
def _aggregate(rows: Sequence[Mapping[str, Any]], splits: Sequence[str]) -> Dict[str, Any]:
    ...
    for split in splits:
        ...
        split_row["mean_hidden_latency_avg_ms"] = _mean(row.get(key, {}).get("hidden_latency_avg_ms") for row in rows)
...
def _render(rows: Sequence[Mapping[str, Any]], aggregate: Mapping[str, Any], splits: Sequence[str]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f"cache-val hidden_FDE_min: {row.get('cache_val_hidden_FDE_min')}")
        ...
        lines.append(f"cache-val hidden_gate_mean: {row.get('cache_val_hidden_gate_mean')}")
        lines.append(f"cache-val hidden_delta_l2_mean: {row.get('cache_val_hidden_delta_l2_mean')}")
        lines.append("")
        for split in splits:
            official = row.get(f"official_{split}", {})
            lines.append(f"-- official {split} HiddenAdapter - Fast --")
            for metric in METRICS:
                lines.append(
                    f"d{metric}: {_fmt(official.get(f'd{metric}'), signed=True)}  "
                    f"hidden_adapter={_fmt(official.get(f'hidden_adapter_{metric}'))}  "
                    f"fast={_fmt(official.get(f'fast_{metric}'))}"
                )
            lines.append(f"slow_FDE_min: {official.get('slow_FDE_min')}")
            lines.append(f"hidden_latency_avg_ms: {official.get('hidden_latency_avg_ms')}")
    ...
    for split in splits:
        ...
        lines.append(f"mean hidden_latency_avg_ms: {_fmt(mean.get('mean_hidden_latency_avg_ms'))}")
...
def main() -> None:
    ...
    payload = {
        "meta": {
            "script": "trustmoe_traj.scripts.summarize_student_hidden_adapter",
