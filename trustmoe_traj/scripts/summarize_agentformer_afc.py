"""Summarize AgentFormer AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize AgentFormer AFC results.")
    ...
    parser.add_argument("--branch-name", type=str, default="agentformer20_pred")
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_agentformer_afc.json",
    )
    return parser
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = []
    lines.append("# AgentFormer AFC Summary")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("- This table evaluates AgentFormer/DLow K=20 stochastic prediction sets under the same AFC protocol as MoFlow, Social-STGCNN, and PECNet.")
    lines.append("- The branch is the exported sampled prediction set; AFC-WMR and AFC-Chamfer are the main external-model evidence for metric distinguishability.")
    if missing:
        lines.append("")
...
def main() -> None:
    ...
    _write_csv(output_dir / "agentformer_afc_summary.csv", rows)
    _write_md(output_dir / "agentformer_afc_summary.md", rows, missing)
    (output_dir / "agentformer_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'agentformer_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'agentformer_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'agentformer_afc_summary.json').as_posix()}")
