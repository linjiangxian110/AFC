...
# Train and evaluate V59C Anchor-hard AFC-aware residual refinement with teacher spread floor.
# The script intentionally resolves slow/data paths from existing V58M eval JSONs,
# while the new V59C refiner and quality scorer checkpoints are passed explicitly.
...
RUN_ID="${RUN_ID:-$(date +%Y%m%d)_v59c_anchor_hard_afc_floor}"
...
LAMBDA_V59_ANCHOR_KEEP="${LAMBDA_V59_ANCHOR_KEEP:-1.0}"
LAMBDA_V59_SPREAD_FLOOR="${LAMBDA_V59_SPREAD_FLOOR:-1.0}"
V59_SPREAD_FLOOR_ENDPOINT_RATIO="${V59_SPREAD_FLOOR_ENDPOINT_RATIO:-0.85}"
V59_SPREAD_FLOOR_TRAJECTORY_RATIO="${V59_SPREAD_FLOOR_TRAJECTORY_RATIO:-0.85}"
...
ANCHOR_QD_SELECTION_MODE="${ANCHOR_QD_SELECTION_MODE:-set_coverage_floor}"
...
ANCHOR_QD_SPREAD_FLOOR_ENDPOINT_RATIO="${ANCHOR_QD_SPREAD_FLOOR_ENDPOINT_RATIO:-0.85}"
ANCHOR_QD_SPREAD_FLOOR_TRAJECTORY_RATIO="${ANCHOR_QD_SPREAD_FLOOR_TRAJECTORY_RATIO:-0.85}"
...
run_dataset_seed() {
  ...
  local refiner_ckpt="$refiner_dir/v59c_refiner_best.pt"
  ...
  if [[ "$TRAIN_REFINER" == "1" && ( "$FORCE" == "1" || ! -f "$refiner_ckpt" ) ]]; then
    ...
    CUDA_VISIBLE_DEVICES="$GPU" "$PY" -m trustmoe_traj.scripts.train_social_cvae_refiner \
      ...
      --lambda-v59-anchor-keep "$LAMBDA_V59_ANCHOR_KEEP" \
      --lambda-v59-spread-floor "$LAMBDA_V59_SPREAD_FLOOR" \
      --v59-spread-floor-endpoint-ratio "$V59_SPREAD_FLOOR_ENDPOINT_RATIO" \
      --v59-spread-floor-trajectory-ratio "$V59_SPREAD_FLOOR_TRAJECTORY_RATIO" \
  ...
  if [[ "$TRAIN_QUALITY" == "1" && ( "$FORCE" == "1" || ! -f "$quality_ckpt" ) ]]; then
    ...
    log "TRAIN V59C quality scorer dataset=$dataset seed=$seed"
  ...
  if [[ "$RUN_EVAL" == "1" ]]; then
    ...
    for threshold in $THRESHOLDS; do
      ...
      eval_prefix="v59c_${dataset}_anchor_hard_afc_floor_${tag}"
      for split in $SPLITS; do
        ...
        log "EVAL V59C dataset=$dataset seed=$seed split=$split threshold=$threshold"
...
log "START V59C Anchor-hard AFC floor experiment"
log "MAIN=$MAIN"
log "PY=$PY"
...
if [[ "$RUN_SUMMARY" == "1" ]]; then
  ...
  "$PY" -m trustmoe_traj.scripts.analyze_v58m_afc_mvp \
    ...
    --eval-prefix-template "v59c_{dataset}_anchor_hard_afc_floor_{tag}" 2>&1 | tee "$LOG_ROOT/analysis_afc_mvp.log"
  "$PY" -m trustmoe_traj.scripts.analyze_v58m_qd_supplement \
    ...
    --threshold-tags "$(threshold_tags_csv)" \
    --run-prefix-template "${RUN_ID}_{dataset}" \
    --eval-prefix-template "v59c_{dataset}_anchor_hard_afc_floor_{tag}" 2>&1 | tee "$LOG_ROOT/analysis_qd_supplement.log"
fi
