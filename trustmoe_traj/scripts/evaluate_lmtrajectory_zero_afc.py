"""Evaluate LMTrajectory zero-shot prediction bundles with the standard ETH-UCY AFC bank."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate LMTrajectory-ZERO exported bundle with AFC metrics.")
...
def main() -> None:
    ...
    print(
        "[evaluate_lmtrajectory_zero_afc] "
    ...
    for index, record in enumerate(records):
        ...
        if (index + 1) % 100 == 0:
            print(f"[evaluate_lmtrajectory_zero_afc] evaluated_records={index + 1}/{len(records)}")
    ...
    output = {
        "meta": {
            "script": "trustmoe_traj.scripts.evaluate_lmtrajectory_zero_afc",
            "baseline": "LMTrajectory-ZERO",
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "dataset": dataset,
            "split": split,
            "branch": branch,
            "model_name": bundle_meta.get("model_name"),
