"""Aggregate V23 teacher fine-tune experiment outputs."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize teacher fine-tune outputs.")
    ...
    parser.add_argument("--eval-file-prefix", type=str, default="v23a_official")
...
def _seed_row(args: argparse.Namespace, project_root: Path, seed: int, splits: Sequence[str]) -> Dict[str, Any]:
    ...
    if isinstance(best_val, Mapping):
        row["process_val_finetuned_FDE_min"] = best_val.get("finetuned_FDE_min")
        row["process_val_base_FDE_min"] = best_val.get("base_FDE_min")
        row["process_val_dFDE_min"] = best_val.get("dFDE_min")
        row["process_val_dMissRate"] = best_val.get("dMissRate")
        row["process_val_base_best_hurt_mean"] = best_val.get("base_best_hurt_mean")
        row["process_val_base_best_worse_rate"] = best_val.get("base_best_worse_rate")
        row["process_val_temporal_energy_risk_mean"] = best_val.get("temporal_energy_risk_mean")
        row["process_val_temporal_energy_gt_weight_mean"] = best_val.get("temporal_energy_gt_weight_mean")
    ...
    for split in splits:
        ...
        if isinstance(metrics, Mapping):
            for metric in METRICS:
                finetuned = _metric(metrics, "teacher_finetune_pred", metric)
                slow = _metric(metrics, "slow_pred", metric)
                split_row[f"teacher_finetune_{metric}"] = finetuned
                split_row[f"slow_{metric}"] = slow
                split_row[f"d{metric}"] = None if finetuned is None or slow is None else finetuned - slow
            split_row["fast_FDE_min"] = _metric(metrics, "fast_pred", "FDE_min")
            split_row["teacher_finetune_latency_avg_ms"] = _num(
                metrics.get("teacher_finetune_pred_latency_avg_ms")
            )
...
def _aggregate(rows: Sequence[Mapping[str, Any]], splits: Sequence[str]) -> Dict[str, Any]:
    ...
    for split in splits:
        ...
        split_row["mean_teacher_finetune_latency_avg_ms"] = _mean(
            row.get(key, {}).get("teacher_finetune_latency_avg_ms") for row in rows
        )
...
def _render(rows: Sequence[Mapping[str, Any]], aggregate: Mapping[str, Any], splits: Sequence[str]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f"process-val finetuned_FDE_min: {row.get('process_val_finetuned_FDE_min')}")
        lines.append(f"process-val base_FDE_min: {row.get('process_val_base_FDE_min')}")
        lines.append(f"process-val dFDE_min: {_fmt(row.get('process_val_dFDE_min'), signed=True)}")
        lines.append(f"process-val dMissRate: {_fmt(row.get('process_val_dMissRate'), signed=True)}")
        lines.append(f"process-val base_best_hurt_mean: {row.get('process_val_base_best_hurt_mean')}")
        lines.append(f"process-val base_best_worse_rate: {row.get('process_val_base_best_worse_rate')}")
        if row.get("process_val_temporal_energy_risk_mean") is not None:
            lines.append(f"process-val temporal_energy_risk_mean: {row.get('process_val_temporal_energy_risk_mean')}")
        if row.get("process_val_temporal_energy_gt_weight_mean") is not None:
            lines.append(
                "process-val temporal_energy_gt_weight_mean: "
                f"{row.get('process_val_temporal_energy_gt_weight_mean')}"
            )
        lines.append("")
        for split in splits:
            official = row.get(f"official_{split}", {})
            lines.append(f"-- official {split} TeacherFinetune - Slow --")
            for metric in METRICS:
                lines.append(
                    f"d{metric}: {_fmt(official.get(f'd{metric}'), signed=True)}  "
                    f"teacher_finetune={_fmt(official.get(f'teacher_finetune_{metric}'))}  "
                    f"slow={_fmt(official.get(f'slow_{metric}'))}"
                )
            lines.append(f"fast_FDE_min: {official.get('fast_FDE_min')}")
            lines.append(f"teacher_finetune_latency_avg_ms: {official.get('teacher_finetune_latency_avg_ms')}")
    ...
    for split in splits:
        ...
        lines.append(f"mean teacher_finetune_latency_avg_ms: {_fmt(mean.get('mean_teacher_finetune_latency_avg_ms'))}")
...
def main() -> None:
    ...
    payload = {
        "meta": {
            "script": "trustmoe_traj.scripts.summarize_teacher_finetune",
