"""Evaluate MID prediction bundles with the standard ETH-UCY AFC bank."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate MID exported bundle with AFC metrics.")
...
def main() -> None:
    ...
    for index, record in enumerate(records):
        ...
        if (index + 1) % 50 == 0:
            print(f"[evaluate_mid_afc] evaluated_records={index + 1}/{len(records)}")
    ...
    output = {
        "meta": {
            "script": "trustmoe_traj.scripts.evaluate_mid_afc",
            "baseline": "MID",
