"""Summarize EigenTrajectory AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize EigenTrajectory AFC results.")
    ...
    parser.add_argument("--branch-name", type=str, default="eigentrajectory_lbebm20_pred")
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_eigentrajectory_afc.json",
    )
    return parser
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = ["# EigenTrajectory AFC Summary", ""]
    ...
    lines.extend(["", "## Interpretation", "", "- This table evaluates EigenTrajectory K=20 prediction sets under the standard AFC protocol."])
    if missing:
...
def main() -> None:
    ...
    _write_csv(output_dir / "eigentrajectory_afc_summary.csv", rows)
    _write_md(output_dir / "eigentrajectory_afc_summary.md", rows, missing)
    (output_dir / "eigentrajectory_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'eigentrajectory_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'eigentrajectory_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'eigentrajectory_afc_summary.json').as_posix()}")
