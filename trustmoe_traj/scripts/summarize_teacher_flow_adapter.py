"""Aggregate teacher flow adapter experiment outputs."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize teacher flow adapter outputs.")
    ...
    parser.add_argument("--eval-file-prefix", type=str, default="v22a_official")
...
def _seed_row(args: argparse.Namespace, project_root: Path, seed: int, splits: Sequence[str]) -> Dict[str, Any]:
    ...
    if isinstance(best_val, Mapping):
        row["cache_val_adapter_FDE_min"] = best_val.get("finetuned_FDE_min")
        row["cache_val_dFDE_min"] = best_val.get("dFDE_min")
        row["cache_val_dMissRate"] = best_val.get("dMissRate")
        row["cache_val_teacher_best_hurt_mean"] = best_val.get("student_best_hurt_mean")
        row["cache_val_teacher_best_worse_rate"] = best_val.get("student_best_worse_rate")
        row["cache_val_flow_gate_mean"] = best_val.get("flow_gate_mean")
        row["cache_val_flow_delta_l2_mean"] = best_val.get("flow_delta_l2_mean")
        row["cache_val_temporal_energy_risk_mean"] = best_val.get("temporal_energy_risk_mean")
        row["cache_val_temporal_energy_gate_target_mean"] = best_val.get("temporal_energy_gate_target_mean")
    ...
    for split in splits:
        ...
        if isinstance(metrics, Mapping):
            for metric in METRICS:
                adapted = _metric(metrics, "teacher_flow_adapter_pred", metric)
                slow = _metric(metrics, "slow_pred", metric)
                split_row[f"teacher_flow_adapter_{metric}"] = adapted
                split_row[f"slow_{metric}"] = slow
                split_row[f"d{metric}"] = None if adapted is None or slow is None else adapted - slow
            split_row["fast_FDE_min"] = _metric(metrics, "fast_pred", "FDE_min")
            split_row["teacher_flow_latency_avg_ms"] = _num(
                metrics.get("teacher_flow_adapter_pred_latency_avg_ms")
            )
            split_row["slow_latency_avg_ms"] = _num(metrics.get("slow_pred_latency_avg_ms"))
            split_row["gate_mean"] = _num(metrics.get("teacher_flow_adapter_gate_mean"))
            split_row["delta_l2_mean"] = _num(metrics.get("teacher_flow_adapter_delta_l2_mean"))
...
def _aggregate(rows: Sequence[Mapping[str, Any]], splits: Sequence[str]) -> Dict[str, Any]:
    ...
    for split in splits:
        ...
        split_row["mean_gate"] = _mean(row.get(key, {}).get("gate_mean") for row in rows)
        split_row["mean_delta_l2"] = _mean(row.get(key, {}).get("delta_l2_mean") for row in rows)
        split_row["mean_teacher_flow_latency_avg_ms"] = _mean(
            row.get(key, {}).get("teacher_flow_latency_avg_ms") for row in rows
        )
...
def _render(rows: Sequence[Mapping[str, Any]], aggregate: Mapping[str, Any], splits: Sequence[str]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f"cache-val adapter_FDE_min: {row.get('cache_val_adapter_FDE_min')}")
        ...
        lines.append(f"cache-val teacher_best_hurt_mean: {row.get('cache_val_teacher_best_hurt_mean')}")
        lines.append(f"cache-val teacher_best_worse_rate: {row.get('cache_val_teacher_best_worse_rate')}")
        lines.append(f"cache-val flow_gate_mean: {row.get('cache_val_flow_gate_mean')}")
        lines.append(f"cache-val flow_delta_l2_mean: {row.get('cache_val_flow_delta_l2_mean')}")
        if row.get("cache_val_temporal_energy_risk_mean") is not None:
            lines.append(f"cache-val temporal_energy_risk_mean: {row.get('cache_val_temporal_energy_risk_mean')}")
        if row.get("cache_val_temporal_energy_gate_target_mean") is not None:
            lines.append(
                "cache-val temporal_energy_gate_target_mean: "
                f"{row.get('cache_val_temporal_energy_gate_target_mean')}"
            )
        lines.append("")
        for split in splits:
            official = row.get(f"official_{split}", {})
            lines.append(f"-- official {split} TeacherFlowAdapter - Slow --")
            for metric in METRICS:
                lines.append(
                    f"d{metric}: {_fmt(official.get(f'd{metric}'), signed=True)}  "
                    f"teacher_flow_adapter={_fmt(official.get(f'teacher_flow_adapter_{metric}'))}  "
                    f"slow={_fmt(official.get(f'slow_{metric}'))}"
                )
            lines.append(f"fast_FDE_min: {official.get('fast_FDE_min')}")
            lines.append(f"teacher_flow_latency_avg_ms: {official.get('teacher_flow_latency_avg_ms')}")
            lines.append(f"slow_latency_avg_ms: {official.get('slow_latency_avg_ms')}")
            lines.append(f"gate_mean: {official.get('gate_mean')}")
    ...
    for split in splits:
        ...
        lines.append(f"mean gate: {_fmt(mean.get('mean_gate'))}")
        lines.append(f"mean delta_l2: {_fmt(mean.get('mean_delta_l2'))}")
        lines.append(f"mean teacher_flow_latency_avg_ms: {_fmt(mean.get('mean_teacher_flow_latency_avg_ms'))}")
...
def main() -> None:
    ...
    payload = {
        "meta": {
            "script": "trustmoe_traj.scripts.summarize_teacher_flow_adapter",
