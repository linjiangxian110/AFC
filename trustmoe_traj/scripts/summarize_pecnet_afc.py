"""Summarize PECNet AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize PECNet AFC results.")
    parser.add_argument("--input-root", type=str, required=True)
    parser.add_argument("--output-dir", type=str, default=None)
    parser.add_argument("--run-id", type=str, required=True)
    parser.add_argument("--dataset-labels", type=str, default="all")
    ...
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_pecnet_afc.json",
    )
    return parser
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = []
    lines.append("# PECNet AFC Summary")
    lines.append("")
    lines.append(
        "| dataset | split | n | ADE_min | FDE_min | ADE_avg | FDE_avg | "
        "AFC-WMR@0.5 | AFC-Precision@0.5 | Unsupported@0.5 | AFC-Chamfer | Endpoint spread | Trajectory spread |"
    )
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for row in rows:
        lines.append(
    ...
    lines.append("")
    dataset_names = {str(row.get("dataset", "")) for row in rows}
    eth_ucy_names = {"eth", "hotel", "univ", "zara1", "zara2"}
    if dataset_names and dataset_names.issubset(eth_ucy_names):
        lines.append("- This table evaluates PECNet on TrustMoE ETH-UCY subset bundles exported by `export_pecnet_eth_predictions.py`.")
    else:
        lines.append("- This table evaluates PECNet on its pooled `all` social dataset unless a dataset-specific bundle is supplied.")
    lines.append("- The branch is the exported K=20 stochastic prediction set, not the official GT-best destination selection.")
    lines.append("- Use AFC-WMR, precision, unsupported ratio, and chamfer as external-model evidence for metric distinguishability.")
    if missing:
        lines.append("")
...
def main() -> None:
    ...
    records, missing = _load_records(
        ...
        dataset_labels=_split_items(args.dataset_labels),
    ...
    _write_csv(output_dir / "pecnet_afc_summary.csv", rows)
    _write_md(output_dir / "pecnet_afc_summary.md", rows, missing)
    (output_dir / "pecnet_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'pecnet_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'pecnet_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'pecnet_afc_summary.json').as_posix()}")
