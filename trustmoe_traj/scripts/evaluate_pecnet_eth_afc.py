"""Evaluate PECNet ETH-UCY prediction bundles with the standard AFC bank."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate PECNet ETH-UCY bundle with AFC metrics.")
...
def main() -> None:
    ...
    for index, record in enumerate(records):
        ...
        if (index + 1) % 50 == 0:
            print(f"[evaluate_pecnet_eth_afc] evaluated_records={index + 1}/{len(records)}")
    ...
    output = {
        "meta": {
            "script": "trustmoe_traj.scripts.evaluate_pecnet_eth_afc",
            "baseline": "PECNet",
