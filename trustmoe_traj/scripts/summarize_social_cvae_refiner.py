"""Aggregate V24 SocialCVAE teacher refiner experiment outputs."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize SocialCVAE teacher refiner outputs.")
    ...
    parser.add_argument("--eval-file-prefix", type=str, default="v24a_official")
...
def _seed_row(args: argparse.Namespace, project_root: Path, seed: int, splits: Sequence[str]) -> Dict[str, Any]:
    ...
    if isinstance(best_val, Mapping):
        row["cache_val_refined_FDE_min"] = best_val.get("refined_FDE_min")
        row["cache_val_base_FDE_min"] = best_val.get("base_FDE_min")
        ...
        row["cache_val_base_best_worse_rate"] = best_val.get("base_best_worse_rate")
        row["cache_val_endpoint_ratio"] = best_val.get("endpoint_ratio")
        row["cache_val_trajectory_ratio"] = best_val.get("trajectory_ratio")
        row["cache_val_delta_l2_mean"] = best_val.get("delta_l2_mean")
        row["cache_val_dynamic_slot_offset_l2_mean"] = best_val.get("dynamic_slot_offset_l2_mean")
        row["cache_val_prior_std_mean"] = best_val.get("prior_std_mean")
        row["cache_val_evaluated_modes"] = best_val.get("evaluated_modes")
        row["cache_val_slot0_FDE_min"] = best_val.get("slot0_FDE_min")
    ...
    for split in splits:
        ...
        if isinstance(metrics, Mapping):
            for metric in METRICS:
                refined = _metric(metrics, "social_cvae_refiner_pred", metric)
                slow = _metric(metrics, "slow_pred", metric)
                split_row[f"social_cvae_refiner_{metric}"] = refined
                split_row[f"slow_{metric}"] = slow
                split_row[f"d{metric}"] = None if refined is None or slow is None else refined - slow
            split_row["fast_FDE_min"] = _metric(metrics, "fast_pred", "FDE_min")
            split_row["social_cvae_refiner_latency_avg_ms"] = _num(
                metrics.get("social_cvae_refiner_pred_latency_avg_ms")
            )
            split_row["slow_latency_avg_ms"] = _num(metrics.get("slow_pred_latency_avg_ms"))
            split_row["delta_l2_mean"] = _num(metrics.get("social_cvae_refiner_delta_l2_mean"))
            split_row["dynamic_slot_offset_l2_mean"] = _num(
                metrics.get("social_cvae_refiner_dynamic_slot_offset_l2_mean")
            )
            split_row["energy_risk_mean"] = _num(metrics.get("social_cvae_refiner_energy_risk_mean"))
...
def _aggregate(rows: Sequence[Mapping[str, Any]], splits: Sequence[str]) -> Dict[str, Any]:
    ...
    for split in splits:
        ...
        split_row["mean_dynamic_slot_offset_l2"] = _mean(
            row.get(key, {}).get("dynamic_slot_offset_l2_mean") for row in rows
        )
        split_row["mean_energy_risk"] = _mean(row.get(key, {}).get("energy_risk_mean") for row in rows)
        split_row["mean_social_cvae_refiner_latency_avg_ms"] = _mean(
            row.get(key, {}).get("social_cvae_refiner_latency_avg_ms") for row in rows
        )
...
def _render(rows: Sequence[Mapping[str, Any]], aggregate: Mapping[str, Any], splits: Sequence[str]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f"cache-val refined_FDE_min: {row.get('cache_val_refined_FDE_min')}")
        lines.append(f"cache-val base_FDE_min: {row.get('cache_val_base_FDE_min')}")
        ...
        lines.append(f"cache-val base_best_hurt_mean: {row.get('cache_val_base_best_hurt_mean')}")
        lines.append(f"cache-val base_best_worse_rate: {row.get('cache_val_base_best_worse_rate')}")
        ...
        lines.append(f"cache-val delta_l2_mean: {row.get('cache_val_delta_l2_mean')}")
        lines.append(f"cache-val dynamic_slot_offset_l2_mean: {row.get('cache_val_dynamic_slot_offset_l2_mean')}")
        lines.append(f"cache-val prior_std_mean: {row.get('cache_val_prior_std_mean')}")
        lines.append(f"cache-val evaluated_modes: {row.get('cache_val_evaluated_modes')}")
        lines.append(f"cache-val slot0_FDE_min: {row.get('cache_val_slot0_FDE_min')}")
        lines.append("")
        for split in splits:
            official = row.get(f"official_{split}", {})
            lines.append(f"-- official {split} SocialCVAERefiner - Slow --")
            for metric in METRICS:
                lines.append(
                    f"d{metric}: {_fmt(official.get(f'd{metric}'), signed=True)}  "
                    f"social_cvae_refiner={_fmt(official.get(f'social_cvae_refiner_{metric}'))}  "
                    f"slow={_fmt(official.get(f'slow_{metric}'))}"
                )
            lines.append(f"fast_FDE_min: {official.get('fast_FDE_min')}")
            lines.append(f"social_cvae_refiner_latency_avg_ms: {official.get('social_cvae_refiner_latency_avg_ms')}")
            ...
            lines.append(f"dynamic_slot_offset_l2_mean: {official.get('dynamic_slot_offset_l2_mean')}")
            lines.append(f"energy_risk_mean: {official.get('energy_risk_mean')}")
            lines.append("")
    ...
    for split in splits:
        ...
        lines.append(f"mean dynamic_slot_offset_l2: {_fmt(mean.get('mean_dynamic_slot_offset_l2'))}")
        lines.append(f"mean energy_risk: {_fmt(mean.get('mean_energy_risk'))}")
        lines.append(
            "mean social_cvae_refiner_latency_avg_ms: "
            f"{_fmt(mean.get('mean_social_cvae_refiner_latency_avg_ms'))}"
        )
...
def main() -> None:
    ...
    payload = {
        "meta": {
            "script": "trustmoe_traj.scripts.summarize_social_cvae_refiner",
