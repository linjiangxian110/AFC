"""Summarize LMTrajectory zero-shot AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize LMTrajectory-ZERO AFC results.")
    ...
    parser.add_argument("--branch-name", type=str, default="lmtrajectory_zero20_pred")
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_lmtrajectory_zero_afc.json",
    )
    return parser
...
def _aggregate(records: Sequence[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for (dataset, split, branch), items in sorted(groups.items()):
        ...
        row: Dict[str, Any] = {"dataset": dataset, "split": split, "branch": branch, "n": len(items), "model_name": ",".join(model_names)}
        for name in DEFAULT_METRICS:
...
def _write_csv(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    preferred = ["dataset", "split", "branch", "model_name", "n"]
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = ["# LMTrajectory-ZERO AFC Summary", ""]
    lines.append("| dataset | split | model | n | ADE_min | FDE_min | ADE_avg | FDE_avg | AFC-WMR@0.5 | AFC-Precision@0.5 | Unsupported@0.5 | AFC-Chamfer | Endpoint spread | Trajectory spread |")
    lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for row in rows:
        lines.append(
            "| "
    ...
    lines.extend(["", "## Interpretation", "", "- This table evaluates LMTrajectory-ZERO K=20 precomputed prediction dumps under the standard AFC protocol."])
    if missing:
...
def main() -> None:
    ...
    _write_csv(output_dir / "lmtrajectory_zero_afc_summary.csv", rows)
    _write_md(output_dir / "lmtrajectory_zero_afc_summary.md", rows, missing)
    (output_dir / "lmtrajectory_zero_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'lmtrajectory_zero_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'lmtrajectory_zero_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'lmtrajectory_zero_afc_summary.json').as_posix()}")
