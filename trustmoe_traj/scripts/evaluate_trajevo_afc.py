"""Evaluate TrajEvo prediction bundles with the standard ETH-UCY AFC bank."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate TrajEvo exported bundle with AFC metrics.")
...
def main() -> None:
    ...
    for index, record in enumerate(records):
        ...
        if (index + 1) % 100 == 0:
            print(f"[evaluate_trajevo_afc] evaluated_records={index + 1}/{len(records)}")
    ...
    output = {
        "meta": {
            "script": "trustmoe_traj.scripts.evaluate_trajevo_afc",
            "baseline": "TrajEvo",
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "dataset": dataset,
            "split": split,
            "branch": branch,
            "model_name": bundle_meta.get("model_name") or "TrajEvo-official-heuristic",
