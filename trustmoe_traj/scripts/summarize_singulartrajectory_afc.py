"""Summarize SingularTrajectory AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize SingularTrajectory AFC results.")
    ...
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_singulartrajectory_afc.json",
    )
    return parser
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = ["# SingularTrajectory AFC Summary", ""]
    ...
    lines.extend(["", "## Interpretation", "", "- This table evaluates SingularTrajectory K=20 prediction sets under the standard AFC protocol."])
    if missing:
...
def main() -> None:
    ...
    _write_csv(output_dir / "singulartrajectory_afc_summary.csv", rows)
    _write_md(output_dir / "singulartrajectory_afc_summary.md", rows, missing)
    (output_dir / "singulartrajectory_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'singulartrajectory_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'singulartrajectory_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'singulartrajectory_afc_summary.json').as_posix()}")
