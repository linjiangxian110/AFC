"""Summarize Social-STGCNN AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize Social-STGCNN AFC results.")
    ...
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_social_stgcnn_afc.json",
    )
    return parser
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = []
    lines.append("# Social-STGCNN AFC Summary")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("- This table evaluates a real external baseline, Social-STGCNN, under the AFC protocol.")
    lines.append("- The prediction branch is the exported K=20 stochastic sample set, not a MoFlow residual-refinement variant.")
    lines.append("- AFC-WMR, precision, unsupported ratio, and chamfer should be compared with MoFlow/CV diagnostic tables as external-model evidence for metric distinguishability.")
    if missing:
        lines.append("")
...
def main() -> None:
    ...
    _write_csv(output_dir / "social_stgcnn_afc_summary.csv", rows)
    _write_md(output_dir / "social_stgcnn_afc_summary.md", rows, missing)
    (output_dir / "social_stgcnn_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'social_stgcnn_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'social_stgcnn_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'social_stgcnn_afc_summary.json').as_posix()}")
