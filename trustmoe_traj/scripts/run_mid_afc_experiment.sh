#!/usr/bin/env bash
set -euo pipefail

MAIN="${MAIN:-/mnt/data/lck/code/TrustMoE-Traj-v38}"
PY="${PY:-}"
if [[ -z "$PY" ]]; then
  if [[ -x /mnt/data/lck/code/moflow/moflow_venv/bin/python ]]; then
    PY=/mnt/data/lck/code/moflow/moflow_venv/bin/python
  else
    PY=python
  fi
fi

find_baseline_root() {
  local name="$1"
  local candidate
  for candidate in \
    "$MAIN/参考/开源基线模型/$name" \
    "$MAIN/baselines/$name" \
    "$MAIN/$name"
  do
    if [[ -d "$candidate" ]]; then
      printf '%s\n' "$candidate"
      return 0
    fi
  done
  candidate="$(find "$MAIN" -maxdepth 5 -type d -name "$name" -print -quit 2>/dev/null || true)"
  if [[ -n "$candidate" && -d "$candidate" ]]; then
    printf '%s\n' "$candidate"
    return 0
  fi
  printf '%s\n' "$MAIN/参考/开源基线模型/$name"
}

GPU="${GPU:-1}"
DEVICE="${DEVICE:-cuda}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d)_mid_afc_exp1_seed0}"
MID_ROOT="${MID_ROOT:-$(find_baseline_root MID)}"
CONFIG_FILE="${CONFIG_FILE:-$MID_ROOT/configs/trustmoe_afc_seed0.yaml}"
CONFIG_NAME="$(basename "$CONFIG_FILE")"
EXP_NAME="${EXP_NAME:-${CONFIG_NAME%.yaml}}"
MODEL_DIR="${MODEL_DIR:-$MID_ROOT/experiments/$EXP_NAME}"
PROCESSED_DIR="${PROCESSED_DIR:-$MID_ROOT/processed_data_noise}"
DATA_ROOT="${DATA_ROOT:-$MAIN/MoFlow/data/eth_ucy/original}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$MAIN/trustmoe_traj/analysis/external_baselines/mid/$RUN_ID}"
LOG_ROOT="${LOG_ROOT:-$OUTPUT_ROOT/logs}"

DATASETS="${DATASETS:-eth hotel univ zara1 zara2}"
SEEDS="${SEEDS:-0}"
SPLITS="${SPLITS:-test}"
K="${K:-20}"
CHECKPOINT_EPOCH="${CHECKPOINT_EPOCH:-90}"
SAMPLING="${SAMPLING:-ddim}"
SAMPLING_STEPS="${SAMPLING_STEPS:-5}"
MID_SDD_SCALE_FACTOR="${MID_SDD_SCALE_FACTOR:-1.0}"
MAX_SCENES="${MAX_SCENES:-}"
MAX_RECORDS="${MAX_RECORDS:-}"
AFC_TOP_M="${AFC_TOP_M:-20}"
AFC_EPS="${AFC_EPS:-0.3,0.5,1.0}"
AFC_MAX_TRAIN_SCENES="${AFC_MAX_TRAIN_SCENES:-}"
AFC_BATCH_SCENES="${AFC_BATCH_SCENES:-64}"
MISS_THRESHOLD="${MISS_THRESHOLD:-2.0}"
RUN_SUMMARY="${RUN_SUMMARY:-1}"
FORCE="${FORCE:-0}"

mkdir -p "$OUTPUT_ROOT" "$LOG_ROOT"

log() {
  local line
  line="[$(date '+%F %T')] $*"
  echo "$line" | tee -a "$LOG_ROOT/manifest.log" >&2
}

optional_arg() {
  local flag="$1"
  local value="$2"
  if [[ -n "$value" ]]; then
    printf '%s\n%s\n' "$flag" "$value"
  fi
}

check_env() {
  log "ENV pwd=$(pwd)"
  log "ENV MAIN=$MAIN"
  log "ENV PY=$PY"
  log "ENV GPU=$GPU CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset>} DEVICE=$DEVICE"
  log "ENV RUN_ID=$RUN_ID"
  log "ENV OUTPUT_ROOT=$OUTPUT_ROOT"
  log "ENV LOG_ROOT=$LOG_ROOT"
  log "ENV MID_ROOT=$MID_ROOT"
  log "ENV CONFIG_FILE=$CONFIG_FILE"
  log "ENV MODEL_DIR=$MODEL_DIR"
  log "ENV PROCESSED_DIR=$PROCESSED_DIR"
  log "ENV DATA_ROOT=$DATA_ROOT"
  log "ENV MID_SDD_SCALE_FACTOR=$MID_SDD_SCALE_FACTOR"
  test "$(pwd)" = "$MAIN" || { echo "[ERR] not in MAIN: $(pwd)" >&2; exit 1; }
  test -x "$PY" || { echo "[ERR] PY not executable: $PY" >&2; exit 1; }
  test -d "$MID_ROOT" || { echo "[ERR] missing MID_ROOT: $MID_ROOT" >&2; exit 1; }
  test -f "$CONFIG_FILE" || { echo "[ERR] missing CONFIG_FILE: $CONFIG_FILE" >&2; exit 1; }
  test -d "$MODEL_DIR" || { echo "[ERR] missing MODEL_DIR: $MODEL_DIR" >&2; exit 1; }
  test -d "$PROCESSED_DIR" || { echo "[ERR] missing PROCESSED_DIR: $PROCESSED_DIR" >&2; exit 1; }
  test -d "$DATA_ROOT" || { echo "[ERR] missing DATA_ROOT: $DATA_ROOT" >&2; exit 1; }
  "$PY" -m py_compile \
    trustmoe_traj/scripts/export_mid_predictions.py \
    trustmoe_traj/scripts/evaluate_mid_afc.py \
    trustmoe_traj/scripts/summarize_mid_afc.py
}

run_one() {
  local dataset="$1"
  local seed="$2"
  local split="$3"
  local checkpoint="$MODEL_DIR/${dataset}_epoch${CHECKPOINT_EPOCH}.pt"
  local run_dir="$OUTPUT_ROOT/${RUN_ID}_${dataset}_seed${seed}"
  local bundle="$run_dir/${dataset}_${split}_mid_k${K}.pt"
  local output_json="$run_dir/${dataset}_${split}_mid_afc.json"
  local max_scene_args=()
  local max_record_args=()
  local afc_max_args=()

  if [[ ! -f "$checkpoint" ]]; then
    echo "Missing MID checkpoint: $checkpoint" >&2
    return 1
  fi
  if [[ ! -f "$PROCESSED_DIR/${dataset}_${split}.pkl" ]]; then
    echo "Missing MID processed data: $PROCESSED_DIR/${dataset}_${split}.pkl" >&2
    return 1
  fi
  mkdir -p "$run_dir"
  mapfile -t max_scene_args < <(optional_arg "--max-scenes" "$MAX_SCENES")
  mapfile -t max_record_args < <(optional_arg "--max-records" "$MAX_RECORDS")
  mapfile -t afc_max_args < <(optional_arg "--afc-max-train-scenes" "$AFC_MAX_TRAIN_SCENES")

  if [[ "$FORCE" == "1" || ! -f "$bundle" ]]; then
    log "EXPORT MID dataset=$dataset seed=$seed split=$split checkpoint=$checkpoint bundle=$bundle"
    CUDA_VISIBLE_DEVICES="$GPU" "$PY" -m trustmoe_traj.scripts.export_mid_predictions \
      --mid-root "$MID_ROOT" \
      --config "$CONFIG_FILE" \
      --model-dir "$MODEL_DIR" \
      --checkpoint-epoch "$CHECKPOINT_EPOCH" \
      --processed-dir "$PROCESSED_DIR" \
      --subset "$dataset" \
      --split "$split" \
      --k "$K" \
      --seed "$seed" \
      --sampling "$SAMPLING" \
      --sampling-steps "$SAMPLING_STEPS" \
      --sdd-scale-factor "$MID_SDD_SCALE_FACTOR" \
      "${max_scene_args[@]}" \
      "${max_record_args[@]}" \
      --output-bundle "$bundle" 2>&1 | tee "$LOG_ROOT/export_${dataset}_seed${seed}_${split}.log"
  else
    log "SKIP export existing bundle=$bundle"
  fi

  if [[ "$FORCE" == "1" || ! -f "$output_json" ]]; then
    log "EVAL_AFC MID dataset=$dataset seed=$seed split=$split output=$output_json"
    "$PY" -m trustmoe_traj.scripts.evaluate_mid_afc \
      --bundle "$bundle" \
      --subset "$dataset" \
      --split "$split" \
      --data-root "$DATA_ROOT" \
      --branch-name "mid${K}_pred" \
      --miss-threshold "$MISS_THRESHOLD" \
      --afc-top-m "$AFC_TOP_M" \
      --afc-eps "$AFC_EPS" \
      --afc-batch-scenes "$AFC_BATCH_SCENES" \
      "${afc_max_args[@]}" \
      --output-json "$output_json" 2>&1 | tee "$LOG_ROOT/eval_${dataset}_seed${seed}_${split}.log"
  else
    log "SKIP eval existing json=$output_json"
  fi
}

check_env
log "START MID AFC experiment"
log "MID_ROOT=$MID_ROOT"
log "CONFIG_FILE=$CONFIG_FILE"
log "MODEL_DIR=$MODEL_DIR CHECKPOINT_EPOCH=$CHECKPOINT_EPOCH"
log "OUTPUT_ROOT=$OUTPUT_ROOT"
log "DATASETS=$DATASETS SEEDS=$SEEDS SPLITS=$SPLITS K=$K GPU=$GPU SAMPLING=$SAMPLING STEPS=$SAMPLING_STEPS"

for dataset in $DATASETS; do
  for seed in $SEEDS; do
    for split in $SPLITS; do
      run_one "$dataset" "$seed" "$split"
    done
  done
done

if [[ "$RUN_SUMMARY" == "1" ]]; then
  log "SUMMARY"
  "$PY" -m trustmoe_traj.scripts.summarize_mid_afc \
    --input-root "$OUTPUT_ROOT" \
    --output-dir "$OUTPUT_ROOT/analysis" \
    --run-id "$RUN_ID" \
    --datasets "$DATASETS" \
    --seeds "$SEEDS" \
    --splits "$SPLITS" \
    --branch-name "mid${K}_pred" 2>&1 | tee "$LOG_ROOT/summary.log"
fi

log "DONE MID AFC experiment"
