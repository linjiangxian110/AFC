#!/usr/bin/env bash
set -euo pipefail

MAIN="${MAIN:-/mnt/data/lck/code/TrustMoE-Traj-v38}"
PY="${PY:-}"
if [[ -z "$PY" ]]; then
  if [[ -x /mnt/data/lck/code/moflow/moflow_venv/bin/python ]]; then
    PY=/mnt/data/lck/code/moflow/moflow_venv/bin/python
  else
    PY=python
  fi
fi

GPU="${GPU:-1}"
DEVICE="${DEVICE:-cuda}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d)_graphtern_afc_exp1_seed0}"
GRAPH_TERN_ROOT="${GRAPH_TERN_ROOT:-$MAIN/参考/开源基线模型/GraphTERN}"
DATA_ROOT="${DATA_ROOT:-$MAIN/MoFlow/data/eth_ucy/original}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$MAIN/trustmoe_traj/analysis/external_baselines/graphtern/$RUN_ID}"
LOG_ROOT="${LOG_ROOT:-$OUTPUT_ROOT/logs}"
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-$GRAPH_TERN_ROOT/checkpoint}"
TAG_PREFIX="${TAG_PREFIX:-graph-tern_}"
TAG_SUFFIX="${TAG_SUFFIX:-_experiment}"

DATASETS="${DATASETS:-eth hotel univ zara1 zara2}"
SEEDS="${SEEDS:-0}"
SPLITS="${SPLITS:-test}"
K="${K:-20}"
MAX_SCENES="${MAX_SCENES:-}"
MAX_RECORDS="${MAX_RECORDS:-}"
AFC_TOP_M="${AFC_TOP_M:-20}"
AFC_EPS="${AFC_EPS:-0.3,0.5,1.0}"
AFC_MAX_TRAIN_SCENES="${AFC_MAX_TRAIN_SCENES:-}"
AFC_BATCH_SCENES="${AFC_BATCH_SCENES:-64}"
MISS_THRESHOLD="${MISS_THRESHOLD:-2.0}"
PRUNING="${PRUNING:-4}"
RUN_SUMMARY="${RUN_SUMMARY:-1}"
FORCE="${FORCE:-0}"

mkdir -p "$OUTPUT_ROOT" "$LOG_ROOT"

log() {
  local line
  line="[$(date '+%F %T')] $*"
  echo "$line" | tee -a "$LOG_ROOT/manifest.log" >&2
}

optional_arg() {
  local flag="$1"
  local value="$2"
  if [[ -n "$value" ]]; then
    printf '%s\n%s\n' "$flag" "$value"
  fi
}

tag_for_dataset() {
  local dataset="$1"
  echo "${TAG_PREFIX}${dataset}${TAG_SUFFIX}"
}

check_env() {
  log "ENV pwd=$(pwd)"
  log "ENV MAIN=$MAIN"
  log "ENV PY=$PY"
  log "ENV GPU=$GPU CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset>} DEVICE=$DEVICE"
  log "ENV RUN_ID=$RUN_ID"
  log "ENV OUTPUT_ROOT=$OUTPUT_ROOT"
  log "ENV LOG_ROOT=$LOG_ROOT"
  log "ENV GRAPH_TERN_ROOT=$GRAPH_TERN_ROOT"
  log "ENV CHECKPOINT_ROOT=$CHECKPOINT_ROOT"
  log "ENV TAG_PREFIX=$TAG_PREFIX TAG_SUFFIX=$TAG_SUFFIX"
  test "$(pwd)" = "$MAIN" || { echo "[ERR] not in MAIN: $(pwd)" >&2; exit 1; }
  test -x "$PY" || { echo "[ERR] PY not executable: $PY" >&2; exit 1; }
  test -d "$GRAPH_TERN_ROOT" || { echo "[ERR] missing GRAPH_TERN_ROOT: $GRAPH_TERN_ROOT" >&2; exit 1; }
  test -d "$DATA_ROOT" || { echo "[ERR] missing DATA_ROOT: $DATA_ROOT" >&2; exit 1; }
  "$PY" - <<'PY'
import numpy as np
import torch
import tqdm
print(f"numpy={np.__version__}")
print(f"torch={torch.__version__}")
print(f"tqdm={tqdm.__version__}")
print(f"cuda_available={torch.cuda.is_available()}")
if torch.cuda.is_available():
    print(f"cuda_device_count={torch.cuda.device_count()}")
PY
  "$PY" -m py_compile \
    trustmoe_traj/scripts/export_graphtern_predictions.py \
    trustmoe_traj/scripts/evaluate_graphtern_afc.py \
    trustmoe_traj/scripts/summarize_graphtern_afc.py
  (
    cd "$GRAPH_TERN_ROOT"
    "$PY" - <<'PY'
from graphtern.model import graph_tern
from utils.dataloader import TrajectoryDataset
print(f"graphtern_model={graph_tern.__name__}")
print(f"graphtern_dataset={TrajectoryDataset.__name__}")
PY
  )
}

run_one() {
  local dataset="$1"
  local seed="$2"
  local split="$3"
  local tag
  tag="$(tag_for_dataset "$dataset")"
  local checkpoint="$CHECKPOINT_ROOT/$tag/${dataset}_best.pth"
  local args_pkl="$CHECKPOINT_ROOT/$tag/args.pkl"
  local run_dir="$OUTPUT_ROOT/${RUN_ID}_${dataset}_seed${seed}"
  local bundle="$run_dir/${dataset}_${split}_graphtern_k${K}.pt"
  local output_json="$run_dir/${dataset}_${split}_graphtern_afc.json"
  local max_scene_args=()
  local max_record_args=()
  local afc_max_args=()
  local branch="graphtern${K}_pred"

  if [[ ! -f "$checkpoint" ]]; then
    echo "Missing GraphTERN checkpoint: $checkpoint" >&2
    return 1
  fi
  if [[ ! -f "$args_pkl" ]]; then
    echo "Missing GraphTERN args.pkl: $args_pkl" >&2
    return 1
  fi
  mkdir -p "$run_dir"
  mapfile -t max_scene_args < <(optional_arg "--max-scenes" "$MAX_SCENES")
  mapfile -t max_record_args < <(optional_arg "--max-records" "$MAX_RECORDS")
  mapfile -t afc_max_args < <(optional_arg "--afc-max-train-scenes" "$AFC_MAX_TRAIN_SCENES")

  if [[ "$FORCE" == "1" || ! -f "$bundle" ]]; then
    log "EXPORT GraphTERN dataset=$dataset seed=$seed split=$split tag=$tag checkpoint=$checkpoint bundle=$bundle"
    CUDA_VISIBLE_DEVICES="$GPU" "$PY" -m trustmoe_traj.scripts.export_graphtern_predictions \
      --graphtern-root "$GRAPH_TERN_ROOT" \
      --checkpoint-root "$CHECKPOINT_ROOT" \
      --tag "$tag" \
      --dataset "$dataset" \
      --split "$split" \
      --k "$K" \
      --seed "$seed" \
      --device "$DEVICE" \
      --pruning "$PRUNING" \
      "${max_scene_args[@]}" \
      --output-bundle "$bundle" 2>&1 | tee "$LOG_ROOT/export_${dataset}_seed${seed}_${split}.log"
  else
    log "SKIP export existing bundle=$bundle"
  fi

  if [[ "$FORCE" == "1" || ! -f "$output_json" ]]; then
    log "EVAL_AFC GraphTERN dataset=$dataset seed=$seed split=$split output=$output_json"
    "$PY" -m trustmoe_traj.scripts.evaluate_graphtern_afc \
      --bundle "$bundle" \
      --dataset "$dataset" \
      --split "$split" \
      --data-root "$DATA_ROOT" \
      --branch-name "$branch" \
      --miss-threshold "$MISS_THRESHOLD" \
      --afc-top-m "$AFC_TOP_M" \
      --afc-eps "$AFC_EPS" \
      --afc-batch-scenes "$AFC_BATCH_SCENES" \
      "${max_record_args[@]}" \
      "${afc_max_args[@]}" \
      --output-json "$output_json" 2>&1 | tee "$LOG_ROOT/eval_${dataset}_seed${seed}_${split}.log"
  else
    log "SKIP eval existing json=$output_json"
  fi
}

check_env
log "START GraphTERN AFC experiment"
log "DATASETS=$DATASETS SEEDS=$SEEDS SPLITS=$SPLITS K=$K GPU=$GPU"

for dataset in $DATASETS; do
  for seed in $SEEDS; do
    for split in $SPLITS; do
      run_one "$dataset" "$seed" "$split"
    done
  done
done

if [[ "$RUN_SUMMARY" == "1" ]]; then
  log "SUMMARY"
  "$PY" -m trustmoe_traj.scripts.summarize_graphtern_afc \
    --input-root "$OUTPUT_ROOT" \
    --output-dir "$OUTPUT_ROOT/analysis" \
    --run-id "$RUN_ID" \
    --datasets "$DATASETS" \
    --seeds "$SEEDS" \
    --splits "$SPLITS" \
    --branch-name "graphtern${K}_pred" 2>&1 | tee "$LOG_ROOT/summary.log"
fi

log "DONE GraphTERN AFC experiment OUTPUT_ROOT=$OUTPUT_ROOT"
