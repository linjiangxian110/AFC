"""Summarize TrajEvo AFC evaluation JSON files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize TrajEvo AFC results.")
    ...
    parser.add_argument("--branch-name", type=str, default="trajevo20_pred")
    parser.add_argument(
        "--file-template",
        type=str,
        default="{input_root}/{run_id}_{dataset}_seed{seed}/{dataset}_{split}_trajevo_afc.json",
    )
    return parser
        ...
        for seed in seeds:
            for split in splits:
                ...
                row: Dict[str, Any] = {
                    ...
                    "split": split,
                    "branch": branch,
                    "model_name": meta.get("model_name") or "TrajEvo-official-heuristic",
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]], missing: Sequence[str]) -> None:
    lines: List[str] = ["# TrajEvo AFC Summary", ""]
    ...
    lines.extend(["", "## Interpretation", "", "- This table evaluates TrajEvo K=20 official evolved heuristics under the standard AFC protocol."])
    if missing:
...
def main() -> None:
    ...
    _write_csv(output_dir / "trajevo_afc_summary.csv", rows)
    _write_md(output_dir / "trajevo_afc_summary.md", rows, missing)
    (output_dir / "trajevo_afc_summary.json").write_text(
        json.dumps({"rows": rows, "records": records, "missing": missing}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"summary_md={(output_dir / 'trajevo_afc_summary.md').as_posix()}")
    print(f"summary_csv={(output_dir / 'trajevo_afc_summary.csv').as_posix()}")
    print(f"summary_json={(output_dir / 'trajevo_afc_summary.json').as_posix()}")
