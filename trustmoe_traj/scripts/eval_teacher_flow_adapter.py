"""Official-style evaluation for teacher flow adapters."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate a teacher-side flow residual adapter.")
...
def _print_delta_summary(metrics: Mapping[str, float]) -> None:
    print("\n[eval_teacher_flow_adapter] TeacherFlowAdapter - Slow")
    for name in METRICS:
        adapted = _metric(metrics, "teacher_flow_adapter_pred", name)
        slow = _metric(metrics, "slow_pred", name)
        delta = None if adapted is None or slow is None else adapted - slow
        ...
        print(f"d{name}: {delta_text}  teacher_flow_adapter={adapted_text}  slow={slow_text}")
    if "teacher_flow_adapter_gate_mean" in metrics:
        print(f"teacher_flow_adapter_gate_mean={metrics['teacher_flow_adapter_gate_mean']:.6f}")
    if "teacher_flow_adapter_delta_l2_mean" in metrics:
        print(f"teacher_flow_adapter_delta_l2_mean={metrics['teacher_flow_adapter_delta_l2_mean']:.6f}")
...
def main() -> None:
    ...
    adapted_predictor = MoFlowSlowPredictor(
        _predictor_cfg(
            args=args,
            agents=agents,
            device=device,
    ...
    adapter, adapter_payload = load_teacher_flow_adapter(args.adapter_checkpoint, map_location=device)
    adapted_predictor.attach_teacher_flow_adapter(adapter)
    fast_predictor = None
    ...
    normalization_stats, normalization_meta = _resolve_normalization_stats(
        ...
        predictors=[item for item in (slow_predictor, adapted_predictor, fast_predictor) if item is not None],
    ...
    adapted_predictor._set_normalization_stats(normalization_stats)
    ...
    accumulators: Dict[str, BranchAccumulator] = {
        ...
        "teacher_flow_adapter_pred": BranchAccumulator("teacher_flow_adapter_pred", args.miss_threshold),
    }
    if fast_predictor is not None:
        accumulators["fast_pred"] = BranchAccumulator("fast_pred", args.miss_threshold)

    print(
        "[eval_teacher_flow_adapter] "
    ...
    if _is_diagnostic_normalization_source(protocol_settings.normalization_source):
        print("[eval_teacher_flow_adapter] warning: selected_samples normalization is diagnostic only")
    ...
    for chunk_index, chunk_pairs in enumerate(chunks, start=1):
        ...
        adapted_batch = batch
        if bool(getattr(adapter.config, "use_temporal_interaction_energy", False)):
            if slow_output.slow_pred is None:
                raise RuntimeError("Slow predictor did not return slow_pred for temporal interaction energy")
            ...
            adapted_batch["teacher_temporal_interaction_energy_features"] = temporal_interaction_energy_features

        _restore_rng(rng_state, device)
        adapted_latencies, adapted_output = _measure_predict_latency_ms(
            lambda: adapted_predictor.predict(adapted_batch, return_all_states=False),
            runs=int(args.latency_runs),
            device=device,
        )
        adapted_payload = {"teacher_flow_adapter_pred": adapted_output.slow_pred}
        adapted_summary = evaluate_model_output(
            adapted_payload,
            batch,
            miss_threshold=float(args.miss_threshold),
            prediction_fields=("teacher_flow_adapter_pred",),
        )
        accumulators["teacher_flow_adapter_pred"].add_chunk(adapted_summary.metrics, adapted_latencies)
    ...
    metrics["teacher_flow_adapter_gate_mean"] = float(sum(gate_means) / len(gate_means)) if gate_means else 0.0
    metrics["teacher_flow_adapter_delta_l2_mean"] = (
    ...
    payload = {
        "meta": {
            "script": "trustmoe_traj.scripts.eval_teacher_flow_adapter",
            "variant": adapter_payload.get("meta", {}).get("variant", "v22a_teacher_flow_adapter"),
